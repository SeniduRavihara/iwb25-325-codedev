...
import { mockContests, mockChallenges, mockContestParticipants } from "@/lib/mock-data"
import { Clock, Users, Trophy, Calendar, Play, Edit, Award } from "lucide-react"
...
export default function ContestPage({ params }: ContestPageProps) {
  ...
  const leaderboard = mockContestParticipants.filter((p) => p.contestId === params.id)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{formatDateTime(contest.startTime)}</div>
                  </div>
                  <div className="text-center">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xs text-muted-foreground">{contestChallenges.length} challenges</div>
                  </div>
                </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {challenge.timeLimit}min • {challenge.successRate}% success
                          </span>
                        </div>
                      </div>
                    </div>
                    <Button variant="outline" size="sm" asChild>
                      <Link href={`/challenges/${challenge.id}`}>View Problem</Link>
                    </Button>
                  </div>
                ))}
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: contest.rules }} />
              </CardContent>
            </Card>
          </div>
                ...
                <CardContent>
                  ...
                  {leaderboard.length > 5 && (
                    <Button variant="outline" size="sm" className="w-full mt-4 bg-transparent" asChild>
                      <Link href={`/contests/${contest.id}/leaderboard`}>View Full Leaderboard</Link>
                    </Button>
                  )}
                </CardContent>
              </Card>
